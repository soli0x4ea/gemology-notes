...
# 额外品种（参考表中没有） - 但已在映射中，不需要额外添加
# 但需要确保硬度和稀缺性正确，从笔记中读取
def get_hardness_scarcity(name):
    # 默认值
    default_hardness = ""
    default_scarcity = ""
    if name == "方柱石":
        default_hardness = "5-6"
        default_scarcity = "稀有"
    elif name == "黝帘石":
        default_hardness = "6-7"
        default_scarcity = "一般"
    return default_hardness, default_scarcity

# 更新矿物的硬度和稀缺性（如果缺失）
for m in minerals:
    if m['name'] in main_mapping or m['name'] in sub_mapping:
        # 保留原有值
        pass

# 添加方柱石和黝帘石（如果不在列表中）
existing_names = {m['name'] for m in minerals}
for name in ["方柱石", "黝帘石"]:
    if name not in existing_names:
        hardness, scarcity = get_hardness_scarcity(name)
        minerals.append({"name": name, "hardness": hardness, "scarcity": scarcity})
